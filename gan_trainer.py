...
class Trainer():
    def __init__(self, generator, discriminator, trainloader, testloader, device, epochs, lr, z_dim, sample_size, sample_path, model_path, conditional=False, g_per_d=1):
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            for i, (real_images, real_number) in enumerate(self.trainloader):
                ...
                if (i + 1) % 100 == 0:
                    dg_z = self.discriminator(fake_images, real_number).mean().item()
                    d_x = self.discriminator(real_images, real_number).mean().item()
                    print('Epoch [{}/{}], Step [{}/{}], d_loss: {:.4f}, g_loss: {:.4f}, D(x): {:.2f}, D(G(z)): {:.2f}'.format(epoch, self.epochs, i + 1, len(self.trainloader), d_loss.item(), g_loss.item(), d_x, dg_z))
